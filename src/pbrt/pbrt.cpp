...
void InitPBRT(const PBRTOptions &opt) {
	...
	if (!Options->customLogFile.empty()) {
		Options->customLogStream = std::make_shared<std::ofstream>();
		Options->customLogStream->open(Options->customLogFile, std::ofstream::out | std::ofstream::app);
	}
	...
	if (Options->useGPU && Options->gpuDevice &&
		...
		// gets going...
		std::string env =
			StringPrintf("CUDA_VISIBLE_DEVICES=%d", *Options->gpuDevice);
		_putenv(env.c_str());
	...
	int nThreads =
		Options->nThreads != 0 ? Options->nThreads : AvailableCores();
...
void CleanupPBRT() {
	if (Options->customLogStream && Options->customLogStream->is_open()) {
		Options->customLogStream->close();
	}
