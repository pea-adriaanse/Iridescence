...
class PyramidTest : public testing::Test {
  protected:
	...
	PyramidTest() {
		this->brdf = PyramidBRDF(1.0, 54.7, 1.0, false, false, "none", Vector3f(0,0,0), std::string(""));
...
TEST_F(PyramidTest, CalcReflectDist2) {
	this->brdf = PyramidBRDF(1.0, 45.0, 2, false, false, "none", Vector3f(0,0,0), std::string(""));
	PyramidBRDF::ReflectDist dist;
	Vector3f dir = Vector3f(0,0,1);
	brdf.calcReflectDist(&dist, dir);
