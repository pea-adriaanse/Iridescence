...
class LambertianMaterial {
  public:
	using BxDF = LambertianBRDF;
	using BSSRDF = void;

	static const char *Name() { return "LambertianMaterial"; }

	static LambertianMaterial *Create(
		const TextureParameterDictionary &parameters, Image *normalMap,
		const FileLoc *loc, Allocator alloc);

	LambertianMaterial(Image *normalMap, FloatTexture reflectance)
