...
PyramidMaterial* PyramidMaterial::Create(
	...
	if (_reflectCountInt <= 0)
		parameters.ReportError("reflectCount",
							   "reflectCount cannot be zero or negative");
	...
	if (_reflectCountClamped != _reflectCountInt)
		fprintf(stderr, "Clamped reflectCount %i to %i", _reflectCountInt,
				_reflectCountClamped);

	bool _shadowPaul = parameters.GetOneBool("shadowPaul", false);
	bool _coating = parameters.GetOneBool("coating", false);
	bool _rebounce = parameters.GetOneBool("rebounce", false);
	Float _reflectance =
		Clamp(parameters.GetOneFloat("reflectance", 1.0), 0.0, 1.0);
	std::string _setting = parameters.GetOneString("setting", "none");
	Vector3f _woVector = parameters.GetOneVector3f("wo", Vector3f(0, 0, 0));
	std::string _distOutFile =
		parameters.GetOneString("distOutFile", std::string("dist.csv"));

	return alloc.new_object<PyramidMaterial>(
		normalMap, _peakHeight, _angle, _reflectCountClamped, _shadowPaul, _coating,
		_rebounce, _reflectance, _setting, _woVector, _distOutFile);
}
...
Float fittedReflectance(Float lambda, Float theta) {
	theta = theta * 180 / PiOver2;	// To Degrees
	if (theta > 80)
		theta = 80;
	return baseRadiance(theta) +
		   peakAmplitude(theta) *
			   std::exp(-std::pow(std::pow(lambda - peakCenter(theta), 2) /
